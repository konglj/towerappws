...
public class OrderServiceImpl implements OrderService {
	 ...
	 * @return
	 */
	private int insertShInfo(int userid, int orderid, int state,String content,int province) {
		// 插入审核信息（添加订单信息）
	...
	public Object addOrder(int userid, int towerid) {
		...
		// 插入审核信息（添加订单信息）
		count = insertShInfo(userid, order.getId(), OrderState.qd,StrUtil.addorder_content,towerBaseInfo.getTowerprovince());
		if (count == 0)
			throw new RuntimeException();
	...
	public Map getOrders(OrderPage orderPage) {
		...
		if(orderPage.getTowercity()>0&&orderPage.getTowercity()/100==0)
			orderPage.setTowercity(orderPage.getTowercity()*100+1);
	...
	public synchronized Object updateOrderTargetAddress(int orderid,
		...
		count = insertShInfo(baseInfo.getUserid(), orderid,
				OrderState.target_address_apply,StrUtil.updateaddress_content,baseInfo.getTowerprovince());
		if (count == 0)
			throw new RuntimeException();
		return ResultUtil.generateResponseMsg("1");
	}
	...
	public synchronized Object abandonOrder(int orderid) {
		...
		// 插入审核日志
		// 插入tower_sh_doinfo
		count = insertShInfo(baseInfo.getUserid(), orderid, OrderState.cancel,StrUtil.abondon_content,baseInfo.getTowerprovince());
		if (count == 0)
			throw new RuntimeException();
	...
	public synchronized Object postOrder(int orderid) {
		...
		// 插入审核日志
		// 插入tower_sh_doinfo
		count = insertShInfo(baseInfo.getUserid(), orderid, OrderState.post,StrUtil.post_content,baseInfo.getTowerprovince());
		if (count == 0)
			throw new RuntimeException();
	...
	public synchronized Object delayOrder(int orderid, String delayreason) {
		...
		// 插入审核日志
		// 插入tower_sh_doinfo
		count = insertShInfo(baseInfo.getUserid(), orderid, OrderState.delay,StrUtil.delay_content,baseInfo.getTowerprovince());
		if (count == 0)
			throw new RuntimeException();
